...
on cast(customers.customer_id as string) = subscription_date.customer_id)),
 ...
 membership_revenue as (select sales.customer_id, sales_member_revenue - refund_member_revenue as member_revenue, 0 as ecommerce_revenue  
FROM (select customer_id, SUM(total_amount) as sales_member_revenue from (select *
...
LEFT JOIN (select customer_id, SUM(total_amount) as refund_member_revenue from (select *
  from konnektive.transactions_cleaned 
  where status != '' and status != '0' and status is not null)
...
non_membership_revenue  as (select sales.customer_id,  0 as member_revenue, sales_ecommerce_revenue - refund_ecommerce_revenue as ecommerce_revenue, 
FROM (select customer_id, SUM(total_amount) as sales_ecommerce_revenue from (select *
  from konnektive.transactions_cleaned 
  where status = '' or status = '0' or status is null)
where txn_type = 'SALE' AND response_type = 'SUCCESS'
group by customer_id) sales

LEFT JOIN (select customer_id, SUM(total_amount) as refund_ecommerce_revenue from (select *
  from konnektive.transactions_cleaned 
  where status = '' or status = '0' or status is null)
where txn_type = 'REFUND' AND response_type = 'SUCCESS'
group by customer_id) refund 
